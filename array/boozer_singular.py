...
if len(sys.argv) < 4:
    print("Usage: boozer_singular.py <input_json> <'identity'|'trace'> <num_aux>", file=sys.stderr)
    sys.exit(2)
...
if monodromy_constraint not in ('identity', 'trace'):
    print(f"ERROR: constraint must be 'identity' or 'trace', got {monodromy_constraint!r}",
          file=sys.stderr)
    sys.exit(2)
num_aux = int(sys.argv[3])
if num_aux < 1:
    print(f"ERROR: num_aux must be >= 1, got {num_aux}", file=sys.stderr)
    sys.exit(2)
print(f"Input: {p}")
print(f"Monodromy constraint: {monodromy_constraint}")
print(f"num_aux: {num_aux}")
...
# the same auxiliary coil set. num_aux is fixed for this run (the scan over
# num_aux is done externally by prefix.sh, one value per loop iteration). If any of the three
# solves fails, abort without writing singular.json. Radii are
# np.linspace(0, 1, num_aux+1)[1:]; only Z is fixed during the
# SingularPeriodicFieldLine Newton solve (currents and radii are free).
radii = np.linspace(0, 1, num_aux + 1)[1:]
...
for idx, (xpoint, boozer_surface, (iota, G), ax) in enumerate(
    ...
    if not res['success']:
        print(f"ERROR: idx={idx}, num_aux={num_aux}: SingularPeriodicFieldLine Newton failed")
    ...
    if not bs_res['success']:
        print(f"ERROR: idx={idx}, num_aux={num_aux}: BoozerSurface re-solve failed")
        print("ABORT: singular.json will not be written.")
        sys.exit(1)

    # Reject a self-intersecting polished surface (checked at phi=0).
    if bs_out.surface.is_self_intersecting():
        print(f"ERROR: idx={idx}, num_aux={num_aux}: polished Boozer surface is self-intersecting")
        print("ABORT: singular.json will not be written.")
        sys.exit(1)
    ...
    if not ax_res['success']:
        print(f"ERROR: idx={idx}, num_aux={num_aux}: magnetic-axis re-solve failed")
        print("ABORT: singular.json will not be written.")
        sys.exit(1)

    # Validate the polished xpoint and axis are evaluable at all phi: the
    # downstream RZ fit in mk_manifolds asserts this, so fail fast here.
    ...
    if not all(evaluate_at_phi(xpoint_fl.curve, _phi)[1] for _phi in _qp) or \
       not all(evaluate_at_phi(new_ax.curve, _phi)[1] for _phi in _qp):
        print(f"ERROR: idx={idx}, num_aux={num_aux}: polished xpoint/axis not evaluable at all phi")
        print("ABORT: singular.json will not be written.")
        sys.exit(1)
    ...
    print(f"xpoint idx={idx}: all three solves converged with num_aux={num_aux}")
