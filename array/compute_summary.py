...
def main():
    ...
    # full 2x2 monodromy (tangent) matrix per X-point, matching the naming used
    # in boozer_all.py so the device browser shows it (descriptive: no thr/err).
    for ti, xp in enumerate(xpoints):
        Mm = np.asarray(xp.monodromy_matrix)
        for a in range(2):
            for b in range(2):
                rows.append((f'monodromy_M{a}{b}_idx{ti}', float(Mm[a, b]), None, None))
