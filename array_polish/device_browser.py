...
Each device folder is named with ``key=value`` parameters, e.g.::
    ...
    margin=0p12_well=0.0_Z=0_onvessel=1_distance=1_configID=2_vesselID=2_mono=0/
        xs_output_*.png
        scene_top.png
        scene_left.png
        summary.txt
...
``mono``::
    ...
    0   trace(M) > 2
    1   trace(M) ≈ 2  and  M ≈ I
    3   trace(M) ≈ 2     (trace condition only)
...
PARAM_ORDER = [
    ...
    "distance", "configID", "vesselID", "mono", "ncoils",
]
...
def parse_folder_params(name: str) -> dict[str, str]:
    ...
    # ncoils token in their folder name; treat them as ncoils=0.
    out.setdefault("ncoils", "0")
    return out
