...
ns = 0
nphi = 0
s_pts = np.linspace(0, 1/4., ns, endpoint=False)
phi_pts = np.linspace(0, 1., nphi, endpoint=False)
...
for s in s_pts:
    for phi in phi_pts:
        pc = PillCurve(sdf, s, phi, 50)
        pc.set('radius', 0.05)
        base_trim_curves.append(pc)
#pcs_coils = coils_via_symmetries(base_pcs, base_pcs_currents, 2, True)
#curves_to_vtk([coil.curve for coil in pcs_coils], 'array')
...
print("hooking up the new lattice!")
...
entities = xpoints + boozer_surfaces
sign = np.array([-1.0 for xp in xpoints] + [-1.0 for boozer_surface in boozer_surfaces])
Jcvd = VesselDistance(sdf, entities, sign, CV_THRESHOLD)

entities = base_curves + base_curves
sign = np.array([-1.0 for c in base_curves] + [1.0 for c in base_curves])
Jcoil_on_vessel = VesselDistance(sdf, entities, sign, COV_THRESHOLD)
...
JF = (J_nonQSRatio 
    ...
    + COIL_ON_VESSEL_WEIGHT * Jcoil_on_vessel
...
penalties = {'nonQS': J_nonQSRatio,
        ...
        'entity-to-vessel':COIL_TO_VESSEL_WEIGHT * Jcvd,
        'coil-on-vessel':COIL_ON_VESSEL_WEIGHT * Jcoil_on_vessel,
...
def callback(dofs):
    ...
    md_coil, md_fieldline, md_bs = Jcoil_on_vessel.shortest_distance()
...
for j in range(20):
    ...
    md_coil, md_fieldline, md_bs = Jcvd.shortest_distance()
    ev_err = max(CV_THRESHOLD-np.min([md_fieldline, md_bs]), 0)/np.abs(CV_THRESHOLD)
    
    md_coil, md_fieldline, md_bs = Jcoil_on_vessel.shortest_distance()
    cov_err = max(COV_THRESHOLD-md_coil, 0)/np.abs(COV_THRESHOLD)
    ...
    if curr_err > 0.001:
        CURRENT_WEIGHT*=10
    if ev_err > 0.001 or cv_cons_err > 0.001:
        COIL_TO_VESSEL_WEIGHT*=10
        print("EV ERROR", ev_err)
    if cov_err > 0.001 :
        COIL_ON_VESSEL_WEIGHT*=10
        print("COV ERROR", cov_err)
    if iota_err > 0.001:
        IOTAS_WEIGHT*=10
