...
from simsopt.geo import SurfaceXYZTensorFourier, curves_to_vtk, CurveRZFourier, CurveXYZFourierSymmetries
...
proc0_print(f"Running tracing_particle_0104183.py config={config}", flush=True)
proc0_print("==============================================", flush=True)



[boozer_surfaces, iota_Gs, ma_list] = load(fname)
ma = ma_list[config]
...
ma_fine = CurveXYZFourierSymmetries(quadpoints, ma.order, ma.nfp, ma.stellsym)
ma_fine.x = ma.x

if comm_world is None or comm_world.rank == 0:
    sc_particle.to_vtk(str(OUT_DIR / f'sc_particle_{config}'), h=0.05)
    curves_to_vtk([ma_fine]+curves, str(OUT_DIR / f'curves_{config}'))
...
proc0_print("End of tracing_particle_0104183.py", flush=True)
proc0_print("====================================", flush=True)
