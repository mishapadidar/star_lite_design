...
from simsopt.geo import SurfaceRZFourier, SurfaceXYZTensorFourier, curves_to_vtk, CurveRZFourier, CurveXYZFourierSymmetries
...
# load design
indir = f"../designs/"
[boozer_surfaces, iota_Gs, ma_list, xpoint_curves] = load(indir + "designB_after_forces_opt_19.json")
savedir = "./output/"

for config, (ma, boozer_surface) in enumerate(zip(ma_list, boozer_surfaces)):
    ma = ma.curve
    # Directory for output
    OUT_DIR = savedir + f"tracing_particle_{config}/"
    ...
    quadpoints = np.linspace(0, 1/ma.nfp, 512, endpoint=False)
