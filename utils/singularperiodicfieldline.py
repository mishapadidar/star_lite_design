...
__all__ = ['SingularPeriodicFieldline', 'DependentMu', 'AuxCoilDistance',
...
class SingularPeriodicFieldline(Optimizable):
    ...
    Typical use::
        ...
        fl = SingularPeriodicFieldline(biotsavart, curve, mu0, options=..., stellsym_aux=...)
...
class DependentMu(Optimizable):
    ...
    :class:`SingularPeriodicFieldline`.
    ...
    Typical use::
        ...
        fl = SingularPeriodicFieldline(bs, curve, mu0, ...)
...
class AuxCoilDistance(Optimizable):
    ...
    of one :class:`SingularPeriodicFieldline` at least ``threshold`` away
    ...
    so the modular<->aux penalty is the usual squared one-sided violation, summed
    over every modular-curve quadrature point and every aux circle,

        J_mod = sum max(threshold - d, 0)^2.

    It ALSO keeps the aux coils clear of EACH OTHER (separation >= aux_threshold).
    Two aux coils are coaxial circles (radius r_a at height z_a, r_b at z_b), so
    their closest-point distance is closed form,

        d_ab = sqrt((r_a - r_b)^2 + (z_a - z_b)^2),

    which is |r_a - r_b| for two circles in the same plane and
    sqrt((r_a - r_b)^2 + (2Z)^2) across the +Z / -Z planes.  The aux<->aux term is

        J_aux = sum_{a<b} max(aux_threshold - d_ab, 0)^2,
    ...
    Typical use::
        ...
        Jdist = AuxCoilDistance(fl, modular_curves, threshold=0.15)
        Jdist.J()                      # sum of squared clearance violations
        Jdist.shortest_distance()      # min modular<->aux center-line distance
        Jdist.shortest_aux_distance()  # min aux<->aux distance (for callbacks)
    """

    def __init__(self, fl, curves, threshold, aux_threshold=None):
    ...
    def shortest_aux_distance(self):
        """Minimum distance between two distinct AUXILIARY coils (coaxial
        circles at heights +-Z); inf if there is only one aux circle."""
        ...
        for i in range(len(circ)):
            ...
            for j in range(i + 1, len(circ)):
                rb, zb = circ[j]
                dmin = min(dmin, float(np.sqrt((ra - rb) ** 2 + (za - zb) ** 2)))
        return dmin
