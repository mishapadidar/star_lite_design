...
from star_lite_design.utils.pillpipevessel import PillPipeVesselDistance
import yaml
...
data = load(f"./{current_bound}/design{design}_after_currents_opt_9.json")
...
config = yaml.safe_load(open(f"./{current_bound}/design{design}_after_currents_opt_9.yaml",'r'))
...
Jcvd = PillPipeVesselDistance(bx, by, r, rr, entities, sign, CV_THRESHOLD)
#Jcvd.to_vtk('vessel_pill')
#quit()
...
def callback(dofs):
    ...
    table2.add_row('vessel dimensions', ' '.join([f'{name}={Jcvd.local_full_x[ii]:.3e} ' for ii, name in enumerate(Jcvd.local_dof_names)]))
    table2.add_row('minimum coil-to-coil distance', f'{Jccdist.shortest_distance():.3e}')
    console.print(table2)
    
    Jcvd.to_vtk(OUT_DIR + 'vessel_tmp')
...
#   J3, _ = f(dofs - eps*h)
#   J4, _ = f(dofs - 2*eps*h)
#   fd = (J1*(-1/12) + J2*(8/12) + J3*(-8/12) + J4*(1/12))/eps
#   print("err", (fd - dJh)/np.linalg.norm(dJh), fd, dJh)
#quit()
...
#    )
    ...
    bx, by, r, rr = Jcvd.local_full_x.copy()
    ...
    save([boozer_surfaces, iota_Gs, axes, xpoints], OUT_DIR + f'design{design}_after_forces_opt_{j}.json')
    ...
    # Save to YAML
    with open(OUT_DIR + f'design{design}_after_forces_opt_{j}.yaml', 'w') as f:
