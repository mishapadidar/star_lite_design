...
for ii, bsurf in enumerate(boozer_surfaces):
    ...
    for alpha in [0.9, 0.75, 0.5, 0.25]:
        ...
        surface_hr.x = temp
        bsurf_hr = BoozerSurface(biotsavart_new, surface_hr, Volume(surface_hr), bsurf.targetlabel*alpha, options={'newton_tol':1e-13, 'newton_maxiter':20})
...
magnetic_wells = [MagneticWell([boozer_surfaces_hr[ii], boozer_surfaces[ii]]) for ii in range(len(boozer_surfaces_hr))]
